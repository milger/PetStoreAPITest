...
public class CommonStepDefinitions {
    ...
    public void actorIsAnAPIUser(Actor actor) {
        String theRestApiBaseUrl = EnvironmentVariableReader.serenityProperty(ApiResources.REST_API_BASE_URL_PROPERTY);
    ...
    public void heShouldGetAStatusCode(Actor actor, int statusCode) {
        actor.should(
                seeThat(PetResponse.statusCode(), is(statusCode))
        );
    }
    ...
    public void heShouldGetTheFollowingMessageAsResponse(Actor actor, String expectedResponse) {
        actor.should(
                seeThat(PetResponse.bodyAsString(), is(equalTo(expectedResponse)))
        );
    }
}
